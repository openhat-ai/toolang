...
A non-empty Queue appears expanded above Input without taking focus. It is inset
one terminal-background cell on each side and directly joins full-width Input
without a separator row. The areas retain distinct backgrounds. Expanded Queue
has a centered summary at the
...
selection is shown only by Input's background, without covering the outer
margins; there is no selection marker. The selected entry reserves its right
...
in both modes. Queue has no accent; the outer margins never change with focus
or selection. Key hints stay dim.
