...
accent. The status bar keeps the model name aligned while a four-cell,
background-only comet sweeps across the model name and one surrounding cell on
each side; there is no separate `model` label. A dim elapsed time follows the
model while the Run is active and uses whole-second precision. Short Runs retain
the activity long enough to avoid flashing. This transient UI state is never
