...
cell whose background is the accent color. Start uses the same ANSI cyan as the
banner logo and wordmark, while Steer keeps a distinct accent. Quick-command
bars use the same background-cell treatment with their own accent, and the
bottom `PromptBox` uses the Start accent. The cell replaces
