"""Builtin Telegram channel plugin."""
...
from toolang.concepts.execution import MessageSender
...
class TelegramPlugin:
    """Telegram chat polling and outbound reply plugin."""
    ...
    def __post_init__(self) -> None:
        token = _required_text(self.config.get("token"), name="token")
        self._token = token
        ...
        self._allowed_updates = _normalized_text_list(allowed_updates) or list(DEFAULT_ALLOWED_UPDATES)
    ...
    def _delivery_from_message(
        ...
        *,
        ...
        thread_suffix = (
            f":{message_thread_id}" if isinstance(message_thread_id, int) else ""
        )
    ...
    def _sender_for_chat(self, chat_id: str) -> MessageSender:
        if self._owner_chat_id is not None and chat_id == self._owner_chat_id:
            return cast(MessageSender, "owner")
        if chat_id in self._peer_chat_ids:
            return cast(MessageSender, "peer")
        return cast(MessageSender, "guest")


def create_telegram_plugin(config: dict[str, Any]) -> TelegramPlugin:
    """Create one builtin Telegram plugin instance."""

    return TelegramPlugin(config=dict(config))
