    ...
    *,
    ...
    try:
        ...
        emit_progress(
            ...
            kind="prepare",
            stage="resolve",
            label=f"Failed to resolve {kind} {text}",
            status="failed",
            detail=str(exc),
        )
        raise
    emit_progress(
        ...
        kind="prepare",
        stage="resolve",
        label=f"Resolved {kind} {canonical}",
        status="ok",
        detail=canonical,
    )
    return canonical
...
def materialize_scope(
    ...
    *,
    ...
    progress_id = f"agent:{authored.agent_name}:{scope}"
    emit_progress(
        progress,
        id=progress_id,
        kind="prepare",
...
def _emit_cached_remote_progress(
    ...
    *,
    ...
    emit_progress(
        ...
        kind="prepare",
        stage="materialize",
        label=f"Skipped updating {request.kind} {canonical_ref}",
        status="skipped",
        detail="cached",
    )
    ...
    *,
    ...
    if "://" in ref:
        ...
        emit_progress(
            ...
            kind="prepare",
            stage="fetch",
            label=f"Fetching {request.kind} {ref}...",
            status="pending",
            detail=ref,
        )
        return
    emit_progress(
        ...
        kind="prepare",
        stage="resolve",
        label=f"Resolving {request.kind} {ref}...",
        status="pending",
        detail=ref,
    )
    ...
    if kind == "skill":
        try:
            ...
            emit_progress(
                ...
                kind="prepare",
                stage="fetch",
                label=f"Failed to fetch {kind} {ref}",
                status="failed",
                detail=str(exc),
            )
            raise
