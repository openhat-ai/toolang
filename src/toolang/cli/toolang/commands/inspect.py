...
def _project_step_call(store: RunStore, source: _InspectSubject) -> object:
    ...
    raise UsageError(f"{step.ref} does not support projector call")
...
def _replace_runnable_tokens(operation: str) -> str:
    return " ".join(
        ...
        if word.rsplit("::", 1)[-1].partition(":")[0] in {"flow", "agic"}
        else word
...
def _runnable_activity(runnable: str) -> str:
    kind, separator, name = runnable.rsplit("::", 1)[-1].partition(":")
...
def _parse_inspect_query(values: Sequence[str]) -> _InspectQuery:
    tokens = tuple(values)
    if not tokens:
        raise UsageError("inspect requires a subject")
    projector = (
    ...
    if head in _STATIC_SUBJECT_NAMES:
        ...
        try:
            ...
            raise UsageError(f"invalid pointer: {exc}") from exc
...
def _invalid_child(subject: _InspectSubject, token: str) -> UsageError:
    ...
    return UsageError(f"{label} does not accept a child subject: {token!r}")
