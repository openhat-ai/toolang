...
def control_message(control: ControlRecord) -> MessageTemplate | None:
    ...
    if isinstance(payload, RecallControlPayload):
        ...
        ref = (
            payload.target.workspace + payload.target.path
            if isinstance(payload.target, RulesRecallTarget)
            else payload.target.ref
        )
        tag = _RESOURCE_TAGS.get(payload.target.kind, payload.target.kind)
        return MessageTemplate(
            "user",
            segments,
            tag=tag,
            recall=MessageRecall(ref, payload.revision),
            escape_text=True,
        )
    ...
    return MessageTemplate(
        ...
        escape_text=True,
        tag=tag.removeprefix("toolang:"),
    )


def literal_delta(messages: Sequence[Message]) -> tuple[MessageTemplate, ...]:
    """Record resolved content and its runtime-owned presentation metadata."""
...
def render_delta(
    delta: Sequence[MessageTemplate], resolve: Callable[[TypedRef | ContentRef], object]
) -> tuple[Message, ...]:
...
def _render_segment(
    segment: str | Part | TypedRef | ContentRef,
    ...
    *,
    ...
    if isinstance(segment, ContentRef):
        return deepcopy(tuple(cast(Sequence[Part], resolve(segment))))
...
def resource_frame(
    ...
    tag = "toolang:" + _RESOURCE_TAGS.get(target.kind, target.kind)
    attrs = (
