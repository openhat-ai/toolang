...
def _list_archived_tasks(toolang_root: Path, agent_name: str) -> tuple[TaskEntry, ...]:
    """List archived local tasks for one agent."""

    return _list_tasks(toolang_root, agent_name, lifecycle="archived")


def _list_draft_tasks(toolang_root: Path, agent_name: str) -> tuple[TaskEntry, ...]:
    """List draft local tasks for one agent."""

    return _list_tasks(toolang_root, agent_name, lifecycle="draft")
...
def _find_task(
    ...
    *,
    ...
    for item_lifecycle in lifecycles:
        for entry in _list_tasks(toolang_root, agent_name, lifecycle=item_lifecycle):
            if entry.document.task_id() == task_id:
                return entry
    return None


def _find_archived_task(toolang_root: Path, agent_name: str, task_id: str) -> TaskEntry | None:
    """Find one archived local task by stable task id."""

    return _find_task(toolang_root, agent_name, task_id, lifecycle="archived")
...
def _list_archived_chores(toolang_root: Path, agent_name: str) -> tuple[ChoreEntry, ...]:
    """List archived local chores for one agent."""

    return _list_chores(toolang_root, agent_name, lifecycle="archived")


def _list_draft_chores(toolang_root: Path, agent_name: str) -> tuple[ChoreEntry, ...]:
    """List draft local chores for one agent."""

    return _list_chores(toolang_root, agent_name, lifecycle="draft")
...
def _find_chore(
    ...
    *,
    ...
    lifecycles = _find_lifecycles(lifecycle, include_archived=include_archived)
    for item_lifecycle in lifecycles:
        for entry in _list_chores(toolang_root, agent_name, lifecycle=item_lifecycle):
            if entry.document.chore_id() == chore_id:
                return entry
    return None


def _find_archived_chore(toolang_root: Path, agent_name: str, chore_id: str) -> ChoreEntry | None:
    """Find one archived local chore by stable chore id."""

    return _find_chore(toolang_root, agent_name, chore_id, lifecycle="archived")
...
def _find_job(
    ...
    *,
    ...
    if kind == "task":
        return _find_task(toolang_root, agent_name, job_id, lifecycle=lifecycle)
    return _find_chore(toolang_root, agent_name, job_id, lifecycle=lifecycle)


def _load_task_text(toolang_root: Path, agent_name: str, task_id: str) -> str:
    """Load one task definition."""

    entry = _find_task(toolang_root, agent_name, task_id, lifecycle=None)
    if entry is None:
        raise FileNotFoundError(f"task not found: {task_id}")
    return entry.path.read_text(encoding="utf-8")


def _load_chore_text(toolang_root: Path, agent_name: str, chore_id: str) -> str:
    """Load one chore definition."""

    entry = _find_chore(toolang_root, agent_name, chore_id, lifecycle=None)
    if entry is None:
...
def _clone_task(toolang_root: Path, agent_name: str, task_id: str) -> Path:
...
def _clone_chore(toolang_root: Path, agent_name: str, chore_id: str) -> Path:
...
def _update_task_text(toolang_root: Path, agent_name: str, task_id: str, text: str) -> Path:
    ...
    return _save_task_entry(toolang_root, agent_name, entry, document)


def _update_chore_text(toolang_root: Path, agent_name: str, chore_id: str, text: str) -> Path:
    ...
    return _save_chore_entry(toolang_root, agent_name, entry, document)
...
def _remove_archived_chore(toolang_root: Path, agent_name: str, chore_id: str) -> bool:
    """Remove one archived chore definition."""

    entry = _find_archived_chore(toolang_root, agent_name, chore_id)
    if entry is None:
        return False
    entry.path.unlink()
...
def ready_job_entries(
    ...
    *,
    ...
    if kind in {None, "task"}:
        items.extend(_list_tasks(toolang_root, agent_name))
    if kind in {None, "chore"}:
        items.extend(_list_chores(toolang_root, agent_name))
    return tuple(sorted(items, key=lambda item: str(item.path)))
