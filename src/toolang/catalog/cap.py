...
def _put_local_entry_text(
    ...
    *,
    ...
    entry_path = _local_entry_file_path(
        toolang_root, agent_name, visibility=visibility, kind=kind, name=name
    )
    ref = _local_ref(visibility=visibility, kind=kind, name=name)
    _ensure_name_available(
        toolang_root, agent_name, visibility=visibility, kind=kind, name=name, ref=ref
    )
...
def _remove_local_entry(
    ...
    *,
    ...
    _validate_local_kind(visibility, kind)
    if kind == "skill":
        target = toolang_root / _relative_definition_root(
            agent_name, visibility=visibility, kind=kind, name=name
        )
...
def _cached_remote_entry(
    ...
    if (
        "://" in request.ref
        and _canonicalize_remote_ref(request.kind, request.ref) == cached.ref
    ):
        return cached
...
def _skill_entry(
    ...
    *,
    ...
    root_relative_dir = _relative_definition_root(
...
def _existing_name_refs(
    ...
    *,
    ...
    items.extend(
        _configured_remote_name_refs(
            toolang_root, agent_name, visibility=visibility, kind=kind
        )
    )
    items.extend(
        _prepared_name_refs(toolang_root, agent_name, visibility=visibility, kind=kind)
    )
...
def _prepared_name_refs(
    ...
    *,
    ...
    try:
        ...
        return ()
    return tuple(
        (entry.name, entry.ref) for entry in lock.entries if entry.kind == kind
    )
...
def _local_entry_file_path(
    ...
    *,
    ...
    return toolang_root / _relative_entry_file_path(
        agent_name, visibility=visibility, kind=kind, name=name
    )
...
def _relative_entry_file_path(
    ...
    *,
    ...
    definition_root = _relative_definition_root(
        agent_name, visibility=visibility, kind=kind, name=name
    )
    if kind == "skill":
...
def _resolve_remote_ref(
    kind: EntryKind, ref: str, *, progress: ProgressSink | None = None
) -> str:
    text = ref.strip()
    emit_progress(
        progress,
...
def _remote_ref_candidates(kind: EntryKind, ref: str) -> tuple[str, ...]:
    ...
    if kind == "skill":
        return _remote_ref_existing_repo_candidates(
            ...
            _github_remote_ref_with_default_branch(
                owner, "agent-skills", f"skills/{name}"
            ),
    ...
    if kind == "service":
        return _remote_ref_existing_repo_candidates(
            _github_remote_ref_with_default_branch(
                owner, "agents", f"services/{name}.md"
            ),
            _github_remote_ref_with_default_branch(
                owner, "agent-services", f"{name}.md"
            ),
    ...
    if kind == "prompt":
        return _remote_ref_existing_repo_candidates(
            _github_remote_ref_with_default_branch(
                owner, "agents", f"prompts/{name}.md"
            ),
            _github_remote_ref_with_default_branch(
                owner, "agent-prompts", f"{name}.md"
            ),
    ...
    if kind == "psyche":
        return _remote_ref_existing_repo_candidates(
            _github_remote_ref_with_default_branch(
                owner, "agents", f"psyches/{name}.md"
            ),
            _github_remote_ref_with_default_branch(
                owner, "agent-psyches", f"{name}.md"
            ),
...
def _remote_ref_candidates_for_repo(
    kind: EntryKind, owner: str, repo: str, name: str
) -> tuple[str, ...]:
    return _remote_ref_existing_repo_candidates(
        *(
...
def _remote_path_candidates_for_repo(
    kind: EntryKind, repo: str, name: str
) -> tuple[str, ...]:
    if kind == "skill":
...
def _config_path(
    toolang_root: Path, agent_name: str, *, visibility: PreparedVisibility
) -> Path:
    if visibility == "shared":
...
def _render_config(data: dict[str, object]) -> str:
    ...
    standard_data = {
        key: value for key, value in data.items() if key not in remote_section_names
    }
    remote_data = {
        key: value for key, value in data.items() if key in remote_section_names
    }
        ...
        if isinstance(value, dict):
            ...
            for entry_name, entry_value in sorted(value.items()):
                ...
                rendered = format_inline_table(
                    cast(Mapping[str, object], entry_value), ctx
                )
...
def _config_kind_table_optional(
    data: dict[str, object], kind: EntryKind
) -> dict[str, object] | None:
