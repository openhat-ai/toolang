...
def create_router() -> APIRouter:
    ...
    async def jobs(request: Request, kind: _views.JobKind | None = None) -> dict[str, object]:
        context = request.app.state
        items = _views._job_collection(context, archived=False)
        if kind is not None:
            items = [item for item in items if item["kind"] == kind]
        return {"items": items}

    @router.get("/jobs/archived", tags=["jobs"], summary="List Archived Jobs")
    async def archived_jobs(request: Request, kind: _views.JobKind | None = None) -> dict[str, object]:
        context = request.app.state
        items = _views._job_collection(context, archived=True)
        if kind is not None:
            items = [item for item in items if item["kind"] == kind]
        return {"items": items}
    ...
    async def archived_job_detail(request: Request, job_id: str) -> dict[str, object]:
        ...
        return {"item": _views._job_detail_item(context, kind=kind, entry=entry)}
    ...
    async def chores(request: Request) -> dict[str, object]:
        context = request.app.state
        return {"items": _views._chore_collection(context, archived=False)}
    ...
    async def archived_chores(request: Request) -> dict[str, object]:
        context = request.app.state
        return {"items": _views._chore_collection(context, archived=True)}
