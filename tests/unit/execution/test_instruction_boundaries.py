...
from tests.support.prompting import render_instructions
...
def test_instruction_bodies_cannot_close_their_runtime_owned_wrapper(layer):
    ...
    if layer == "instruct":
        ...
        assert section.attrib["ref"] == 'psyche/precise" & helpful'
...
""",
    ...
    rendered = prompting._render_context(program, program.agics[0], {"body": CONTENT})
...
def test_empty_context_does_not_add_an_empty_data_message(body):
    ...
    assert prompting._render_context(program, program.agics[0], {"body": body}) == ""
