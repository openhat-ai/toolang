...
def test_prepare_does_not_create_missing_agent_source(tmp_path: Path) -> None:
    ...
    assert not (toolang_root / ".prepared").exists()
...
def test_concurrent_processes_publish_one_root_and_home_version(
    ...
    assert len(tuple((toolang_root / ".prepared" / "versions").iterdir())) == 1
    assert (
        len(
            tuple(
                (
...
def test_different_agent_processes_share_one_root_generation(tmp_path: Path) -> None:
    ...
    root_versions = toolang_root / ".prepared" / "versions"
    assert [path.name for path in root_versions.iterdir()] == [alice[1]]
    assert (toolang_root / "agents" / "alice" / ".prepared" / "current").is_file()
    assert (toolang_root / "agents" / "bob" / ".prepared" / "current").is_file()
...
def test_local_change_reuses_unchanged_remote_materialization(
    ...
    assert Path(remote.path).read_text(encoding="utf-8").endswith(
        "Remote content.\n"
    )
